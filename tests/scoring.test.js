...
const verifiedRoomDiscountCandidate = { id: "verified-room-discount", itemCode: "shop:verified", product: { itemCode: "shop:verified", itemName: "80%OFF 箸セット" }, status: "投稿待ち", postStatus: "投稿待ち" };
assert(scoring.applyCouponEvidenceToCandidate(verifiedRoomDiscountCandidate, { regularPrice: 1100, salePrice: 220, discountRate: 80, rateConfirmed: true, discountRateType: "exact", confirmedDiscountLabel: "80%OFF", discountStatus: "confirmed", deadlineConfirmed: false }) === verifiedRoomDiscountCandidate && verifiedRoomDiscountCandidate.rateConfirmed === true && verifiedRoomDiscountCandidate.product.rateConfirmed === true && verifiedRoomDiscountCandidate.regularPrice === 1100 && verifiedRoomDiscountCandidate.salePrice === 220 && verifiedRoomDiscountCandidate.discountStatus === "confirmed", "ROOM discount evidence: verified price/rate is saved on candidate and product");
assert(!verifiedRoomDiscountCandidate.coupon && verifiedRoomDiscountCandidate.deadlineConfirmed === false, "ROOM discount evidence: unverified coupon/deadline remain unset");
const unverifiedRoomDiscountCandidate = { id: "unverified-room-discount", itemCode: "shop:unverified", product: { itemCode: "shop:unverified", itemName: "80%OFF 箸セット" }, status: "投稿待ち", postStatus: "投稿待ち" };
scoring.applyCouponEvidenceToCandidate(unverifiedRoomDiscountCandidate, { discountRate: 80, rateConfirmed: false, discountRateType: "unknown", deadlineConfirmed: false });
assert(unverifiedRoomDiscountCandidate.rateConfirmed === false && unverifiedRoomDiscountCandidate.discountStatus !== "confirmed", "ROOM discount evidence: title-only candidate is not confirmed");
...
assert(scoring.extractDiscountCandidate("最大50%OFF").discountRate === 50, "Coupon candidate E: 最大50%OFF is retained as an unconfirmed candidate");
...
const postedProduct = { itemCode: "posted-1", itemName: "投稿済み商品", itemUrl: "https://example.com/posted" };
scoring.data.history.push({ itemCode: "posted-1", product: postedProduct });
assert(scoring.postedHistoryMatch(postedProduct), "Posted filter A: exact ITEM_CODE matches history");
assert(scoring.filterAvailableProducts([postedProduct, { itemCode: "candidate-1", itemName: "候補商品" }]).length === 1, "Posted filter B: only history item is excluded");
scoring.data.history.pop();
assert(!scoring.postedHistoryMatch({ itemName: "投稿済み商品" }), "Posted filter C: name-only match is not automatically excluded");
const roomCandidate = { id: "room-candidate", destination: "room", itemCode: "room-visible", product: { itemCode: "room-visible", itemName: "未投稿商品" }, status: "投稿待ち" };
const postedTopLevel = { id: "posted-top-level", itemCode: "posted-top-level", product: { itemName: "投稿済み商品A" }, postedAt: "2026-09-26T00:00:00.000Z" };
const postedNested = { id: "posted-nested", product: { itemCode: "posted-nested", itemName: "投稿済み商品B" }, postedAt: "2026-09-26T00:00:00.000Z" };
const threadsCandidate = scoring.createThreadsOnlyCandidate({ itemCode: "posted-top-level", itemName: "Threads専用商品" }, "threads-preserved");
scoring.data.candidates.push(roomCandidate, postedTopLevel, postedNested, threadsCandidate);
scoring.data.history.push({ itemCode: "posted-top-level", product: { itemName: "履歴商品A" } }, { product: { itemCode: "posted-nested", itemName: "履歴商品B" } });
assert(scoring.isVisibleRoomCandidate(roomCandidate), "ROOM filter A: candidate without history remains visible");
assert(!scoring.isVisibleRoomCandidate(postedTopLevel) && !scoring.isVisibleRoomCandidate(postedNested), "ROOM filter B/C: top-level and nested itemCode history matches are hidden");
assert(scoring.data.candidates.includes(postedTopLevel) && scoring.data.candidates.includes(postedNested), "ROOM filter E: posted candidates remain stored");
assert(scoring.data.candidates.filter(scoring.isVisibleRoomCandidate).length === 1, "ROOM filter F/G: posted candidates are excluded from visible ROOM count");
assert(scoring.isThreadsOnlyItem(threadsCandidate) && !scoring.isRoomCandidate(threadsCandidate), "ROOM filter I: Threads-only candidate remains separate");
assert(scoring.getItemCodes({ itemCode: "  normalized-code  ", product: { itemCode: "nested-code" } }).includes("normalized-code"), "ROOM filter itemCode: trim normalization is applied");
assert(!scoring.canSaveRoomCandidate({ itemCode: "posted-top-level" }), "ROOM filter D: posted item cannot be added as a new ROOM candidate");
scoring.data.candidates = scoring.data.candidates.filter((item) => !["room-candidate", "posted-top-level", "posted-nested", "threads-preserved"].includes(item.id));
scoring.data.history = scoring.data.history.filter((item) => !["posted-top-level", "posted-nested"].includes(item.itemCode));
const dealCandidate = scoring.evaluateDealStatus({ itemName: "最後6時間限定 全品15%OFFクーポン発行中" }, "10");
assert(dealCandidate.status === "candidate" && dealCandidate.rate === 15, "Deal 1: title discount is a candidate and remains visible");
assert(scoring.evaluateDealStatus({ itemName: "最後6時間限定 全品15%OFFクーポン発行中" }, "20").status === "unknown", "Deal 2: candidate below threshold is not promoted");
assert(scoring.evaluateDealStatus({ discountRate: 15, rateConfirmed: true, discountRateType: "exact" }, "10").status === "confirmed", "Deal 3: confirmed structured discount matches");
assert(scoring.evaluateDealStatus({ discountRate: 15, rateConfirmed: true, discountRateType: "exact" }, "20").status === "unknown", "Deal 4: confirmed discount below threshold is not a match");
assert(scoring.evaluateDealStatus({ itemName: "ランキング商品" }, "10").status === "unknown", "Deal 6: missing discount remains visible as unknown");
const dealSummary = scoring.summarizeDealStatuses([
  { dealStatus: scoring.evaluateDealStatus({ discountRate: 50, rateConfirmed: true, discountRateType: "exact" }, "50plus") },
  { dealStatus: scoring.evaluateDealStatus({ itemName: "50%OFFクーポン" }, "50plus") },
  { dealStatus: scoring.evaluateDealStatus({ itemName: "通常商品" }, "50plus") }
]);
assert(JSON.stringify(dealSummary) === JSON.stringify({ confirmed: 1, candidate: 1, unknown: 1 }), "Deal summary: confirmed, candidate, and unknown counts are separated");
const discountRates = [20, 30, 40, 50, 60, 70, 80, 90];
discountRates.forEach((rate) => {
  const terms = scoring.buildDiscountSearchTerms(rate);
  assert(terms.includes(`${rate}%OFF`) && terms.includes(`${rate}％OFF`) && terms.includes(`最大${rate}%OFFクーポン`) && terms.includes(`最大${rate}％OFFクーポン`), `Discount terms: ${rate}% variants are generated`);
  assert(scoring.buildDiscountSearchTermsForMinimum(rate).includes(`${rate}%OFF`) && !scoring.buildDiscountSearchTermsForMinimum(rate).includes(`${rate - 10}%OFF`), `Discount minimum: ${rate}% threshold is inclusive`);
});
assert(scoring.evaluateDealStatus({ itemName: "最大80%OFFクーポン対象" }, "80").status === "candidate", "Discount status: maximum-off title remains a candidate");
assert(scoring.evaluateDealStatus({ itemName: "最大80%OFFクーポン対象", rateConfirmed: true, discountRate: 80, discountRateType: "exact", confirmedDiscountLabel: "最大80%OFFクーポン" }, "80").label.includes("最大80%OFFクーポン"), "Discount label: confirmed maximum-off wording is preserved");
assert(scoring.buildDealHeader({ regularPrice: 2080, salePrice: 416, itemPrice: 416, rateConfirmed: true, discountRate: 80, discountRateType: "exact", confirmedDiscountLabel: "80%OFF" }) === "2,080円→416円🉐 80%OFF", "Deal header: confirmed price reduction is generated");
assert(scoring.buildDealHeader({ itemPrice: 416, rateConfirmed: true, discountRate: 80, discountRateType: "exact", confirmedDiscountLabel: "最大80%OFFクーポン", deadlineConfirmed: false, couponDeadline: "9/30" }) === "🉐 最大80%OFFクーポン", "Deal header: unconfirmed deadline is omitted");
